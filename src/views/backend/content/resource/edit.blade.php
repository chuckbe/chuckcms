    ...
    <div class="col-sm-12">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mt-3">
          ...
          <li class="breadcrumb-item active" aria-current="page">Bewerk resource</li>
        </ol>
      </nav>
    </div>
  </div>
                ...
                <div class="row column-seperation">
                  <div class="col-lg-12">
                    <div class="form-group form-group-default required ">
                      <label>Slug</label>
                      <input type="text" class="form-control resource_slug_input" placeholder="slug" id="resource_slug" name="slug[]" value="{{ $resource->slug }}" required>
                    </div>
                    <hr>
                    <div class="resource_field_wrapper" data-lang="{{ $langKey }}">
                      @foreach($resource->json[$langKey] as $rKey => $rValue)
                        <div class="row resource_field_row" data-order="{{ $loop->iteration }}">
                          <div class="col-lg-4">
                            <div class="form-group form-group-default required ">
                              <label>Veld Key</label>
                              <input type="text" class="form-control resource_key" placeholder="key" id="resource_key" name="resource_key[{{ $langKey }}][]" data-order="{{ $loop->iteration }}" value="{{ $rKey }}" required>
                            </div>
                          </div>
                          <div class="col-lg-8">
                            <div class="form-group form-group-default required ">
                              <label>Veld Waarde</label>
                              <input type="text" class="form-control resource_value" placeholder="waarde" id="resource_value" name="resource_value[{{ $langKey }}][]" data-order="{{ $loop->iteration }}" value="{{ $rValue }}" required>
                            </div>
                          </div>
                        </div>
                      @endforeach
